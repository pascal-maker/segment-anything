...
sys.path.append("..")  # Assuming SAM is in a parent directory
...
# Input point and bounding box coordinates (replace if needed)
input_point = np.array([[150, 100]])  # Adjusted based on your suggestion
input_label = np.array([1])  # Label 1 for foreground point
input_box = np.array([50, 50, 250, 150])  # Adjusted based on your suggestion

# Set the image embedding using predictor.set_image
predictor.set_image(image)

# Make predictions with multimask_output for disambiguation
masks, scores, logits = predictor.predict(point_coords=input_point,
                                       point_labels=input_label,
                                       box=input_box,
                                       multimask_output=True)

# Visualize results (adjust figure size as needed)
...
# Iterate through predicted masks and visualize them
for i, (mask, score) in enumerate(zip(masks, scores)):
    plt.subplot(2, 2, i + 2)
    plt.imshow(image)
    show_mask(mask, plt.gca())
    show_box(input_box, plt.gca())  # Show the adjusted bounding box
